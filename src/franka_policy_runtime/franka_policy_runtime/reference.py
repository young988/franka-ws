...
def apply_tcp_delta_in_base_frame(
    ...
    *,
    ...
    """Apply a relative TCP delta in the base/command frame.
...
def step_toward_pose(
    ...
    *,
    ...
    if quat_xyzw.shape != (4,):
        raise ValueError(f"current_quat_xyzw must have shape (4,), got {quat_xyzw.shape}")
    if target_position_arr.shape != (3,):
        raise ValueError(f"target_position must have shape (3,), got {target_position_arr.shape}")
    if target_quat_arr.shape != (4,):
        raise ValueError(f"target_quat_xyzw must have shape (4,), got {target_quat_arr.shape}")
    ...
    if delta_norm > max_translation and delta_norm > 0.0:
    ...
    if angle > max_rotation and angle > 0.0:
